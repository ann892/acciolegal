...
Branding is sourced from firm_profile.FIRM. Default is a fictional firm
(Verita Corporate Counsel) so the demo can be shared publicly without
real client permission. Override via env vars (FIRM_NAME, FIRM_FOUNDER, etc.).
        ...
        <div style='display:flex;align-items:center;justify-content:space-between;
            ...
            <div>
                <div style='font-size:28px;font-weight:800;color:{ACCIO_NAVY};
                            letter-spacing:0.5px;'>{FIRM["name"].upper()}</div>
                <div style='font-size:13px;color:#6A6A6A;margin-top:-2px;'>
                    {active} &nbsp;·&nbsp; {FIRM["founder_name"]}, {FIRM["founder_role"]}
                </div>
            </div>
...
def render_footer() -> None:
    ...
    st.markdown(
        ...
        f"{FIRM['name']} · {FIRM['address']} · "
        f"{FIRM['support_email']} · {FIRM['phone']}"
        f"</div>",
        unsafe_allow_html=True,
    )
...
def page_run_matter() -> None:
    ...
    st.markdown(
        ...
        f"draft a senior-counsel-voice proposal, and render it inline. Then simulate the "
    ...
    if st.button("Generate proposal", type="primary", disabled=("proposal_content" in st.session_state)):
        ...
        with st.spinner("Drafting proposal in the firm's voice using prior-deal style references..."):
            time.sleep(1.0)
    ...
    if "matter_data" in st.session_state and "proposal_content" in st.session_state:
        ...
        if st.button("Generate engagement letter", type="primary", disabled=("el_path" in st.session_state)):
            ...
            with st.spinner("Filling the firm's engagement letter template..."):
                time.sleep(0.8)
        ...
        if "el_path" in st.session_state:
            ...
            if reply_data.get("client_questions"):
                st.warning(
                    "Client asked questions the firm should answer before sending the EL:\n\n"
            ...
            st.caption(
                "The engagement letter uses the firm's existing Word template — only the "
...
def page_settings() -> None:
    ...
    st.markdown(
        f"<div style='background:#F8FAFC;border-left:3px solid {ACCIO_NAVY};"
        f"padding:14px 18px;border-radius:0 6px 6px 0;margin-bottom:24px;'>"
        f"<strong>Connect Gmail.</strong> Click the button below, sign in with "
        f"the Gmail account that should receive AI-drafted proposals and "
        f"engagement letters, and grant access. Drafts generated by "
...
def main() -> None:
    st.set_page_config(
        page_title=f"{FIRM['name']} — Matter Pipeline",
        page_icon="⚖",
        layout="wide",
    )
    ...
    with st.sidebar:
        st.markdown(
            ...
            f"letter-spacing:1px;padding:8px 0 4px 0;'>{FIRM['name'].upper()}</div>"
